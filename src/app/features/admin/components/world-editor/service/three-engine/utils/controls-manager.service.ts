...
export class ControlsManagerService implements OnDestroy {
  ...
  public enableNavigation(): void { this.isOrbitEnabled = true; if (!this.isTouchDevice) { this.isFlyEnabled = true; } this.orbitControls.enabled = true; }
  public disableNavigation(): void { this.isOrbitEnabled = false; this.isFlyEnabled = false; this.orbitControls.enabled = false; this.velocity.set(0, 0, 0); }
  ...
  private handleKeyboardFly = (delta: number, keyMap: Map<string, boolean>): boolean => { const moveDirection = new THREE.Vector3(); const fwd = new THREE.Vector3(); this.camera.getWorldDirection(fwd); const right = new THREE.Vector3().copy(fwd).cross(this.camera.up).normalize(); if (keyMap.get('w')) moveDirection.add(fwd); if (keyMap.get('s')) moveDirection.sub(fwd); if (keyMap.get('a')) moveDirection.sub(right); if (keyMap.get('d')) moveDirection.add(right); if (keyMap.get('e')) moveDirection.y += 1; if (keyMap.get('q')) moveDirection.y -= 1; const didMove = moveDirection.lengthSq() > 0; if (didMove) { this.velocity.lerp(moveDirection.normalize().multiplyScalar(this.MOVEMENT_SPEED), delta * 5); } this.velocity.multiplyScalar(this.DAMPING_FACTOR); const displacement = this.velocity.clone().multiplyScalar(delta); this.camera.position.add(displacement); this.focusPivot.position.add(displacement); this.orbitControls.target.add(displacement); return didMove; };
  private addEventListeners = () => { if (!this.isTouchDevice) { this.domElement.addEventListener('wheel', this.onDocumentMouseWheel, { passive: false }); } this.orbitControls.addEventListener('change', this.syncPivotToControlsTarget); };
  private syncPivotToControlsTarget = (): void => { if (this.focusPivot) { this.focusPivot.position.copy(this.orbitControls.target); } };
  private onDocumentMouseWheel = (event: WheelEvent) => { if (this.isOrbitEnabled) { event.preventDefault(); this.dolly((event.deltaY < 0 ? 1 : -1) * this.DOLLY_SPEED); } };
