// RUTA: .../utils/entity-manager.service.ts
...
export class EntityManagerService {
  ...
  // ⭐️ MEJORA CLAVE: Nuevo método para atenuar o restaurar el brillo de un objeto.
  public setObjectDimmed(uuid: string, isDimmed: boolean): void {
    const instanceInfo = this._findCelestialInstance(uuid);
    if (instanceInfo) {
      instanceInfo.data.isDimmed = isDimmed;
    }
    // Aquí se podría añadir lógica para atenuar también objetos normales si fuera necesario.
  }
  
  // ... (resto de métodos sin cambios) ...
  public publishSceneEntities(): void { const entities: SceneEntity[]=[]; this.scene.children.forEach(object => { if (!object.name.endsWith('_helper') && !this.unselectableNames.includes(object.name) && !object.name.startsWith(CELESTIAL_MESH_PREFIX)) { const apiType=object.userData['apiType'] as SceneEntity['type'] | undefined; entities.push({ uuid: object.uuid, name: object.name, type: apiType || (object instanceof THREE.Camera ? 'Camera' : (object instanceof THREE.Light ? 'Light' : 'Model')) }); } if (object.name.startsWith(CELESTIAL_MESH_PREFIX)) { const allInstanceData: CelestialInstanceData[]=object.userData["celestialData"]; if (allInstanceData) { allInstanceData.forEach(instance => { entities.push({ uuid: instance.originalUuid, name: instance.originalName, type: instance.type as SceneObjectResponse['type'] }); }); } } }); entities.sort((a, b) => a.name.localeCompare(b.name)); setTimeout(() => this.sceneEntities.next(entities)); }
  private _findCelestialInstance(uuid: string): { mesh: THREE.InstancedMesh, instanceIndex: number, data: CelestialInstanceData } | null { for (const object of this.scene.children) { if (object.name.startsWith(CELESTIAL_MESH_PREFIX) && object instanceof THREE.InstancedMesh) { const allInstanceData: CelestialInstanceData[]=object.userData["celestialData"]; if (allInstanceData) { const instanceIndex=allInstanceData.findIndex(d => d.originalUuid===uuid); if (instanceIndex > -1) { return { mesh: object, instanceIndex, data: allInstanceData[instanceIndex] }; } } } } return null; }
  public selectObjectByUuid(uuid: string | null, focusPivot: THREE.Object3D): void { const existingProxy=this.scene.getObjectByName('SelectionProxy'); if (existingProxy) { this.scene.remove(existingProxy); (existingProxy as THREE.Mesh).geometry.dispose(); ((existingProxy as THREE.Mesh).material as THREE.Material).dispose(); } if (!uuid) { this.selectionManager.selectObjects([]); return; } const mainObject=this.scene.getObjectByProperty('uuid', uuid); if (mainObject) { focusPivot.position.copy(mainObject.position); this.selectionManager.selectObjects([mainObject]); return; } const celestialInstance=this._findCelestialInstance(uuid); if (celestialInstance) { const { data }=celestialInstance; const selectionProxy=this.objectManager.createSelectionProxy(); const pos=new THREE.Vector3(), quat=new THREE.Quaternion(), scale=new THREE.Vector3(); data.originalMatrix.decompose(pos, quat, scale); selectionProxy.position.copy(pos); selectionProxy.scale.copy(scale).multiplyScalar(PROXY_SCALE_MULTIPLIER); selectionProxy.uuid=data.originalUuid; this.scene.add(selectionProxy); this.selectionManager.selectObjects([selectionProxy]); focusPivot.position.copy(selectionProxy.position); return; } this.selectionManager.selectObjects([]); }
  ...
  public updateObjectName(uuid: string, newName: string): void { const objectToUpdate: THREE.Object3D | undefined=this.getObjectByUuid(uuid); if (objectToUpdate) { objectToUpdate.name=newName; if (objectToUpdate.userData['helper']) { objectToUpdate.userData['helper'].name=`${newName}_helper`; } } else { const instance=this._findCelestialInstance(uuid); if (instance) { instance.data.originalName=newName; } } this.publishSceneEntities(); }
