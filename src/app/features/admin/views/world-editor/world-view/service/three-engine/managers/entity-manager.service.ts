...
export class EntityManagerService {
  ...
  public selectObjectByUuid(uuid: string | null, focusPivot: THREE.Object3D): void {
    ...
    if (celestialInstance) {
      ...
      // ✨ MEJORA: Usamos data.position y data.scale directamente para asegurar que
      // el proxy se basa en los datos originales, no en una matriz descompuesta.
  ...
  public createOrUpdateHoverProxy(instancedMesh: THREE.InstancedMesh, instanceId: number): THREE.Mesh {
    ...
    if (data) {
      ...
      // ✨ MEJORA: Usamos data.position y data.scale para mayor robustez.
      this.hoverProxy.position.copy(data.position);
